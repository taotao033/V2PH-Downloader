...
> **免责声明**：这些脚本依 GPLv3 授权，按「现状」（AS IS）提供、不作任何担保，**仅供使用者在本机进行个人离线归档之用**。使用者须自行确保其行为符合所在司法管辖区的全部适用法律法规（著作权、隐私与个人信息保护、计算机犯罪及未经授权访问、第三方网站服务条款等），并对所归档内容的来源、版权与合法性自行承担全部责任。在法律允许的最大范围内，开发者不对因使用、误用或无法使用本脚本所导致的任何直接或间接损害（含账号封禁、法律诉讼、第三方索赔、行政或刑事处分等）承担责任。完整条款见项目根目录 [`README.md`](../README.md)。
...
封面通过 `og:image` meta 标签抓取（CDN 缩略图），保存到 `<destination>/_covers/<album_slug>.jpg`（可用 `--cover-dir` 改路径）。路径同步写入 `albums.cover_local_path` 列。
...
# 先跑一小批（50 条）验证提取正确（同时下载封面）
python scripts\backfill_album_profiles.py -d "D:\v2ph_archive" --limit 50

# 全量跑（3000+ 条，每条默认 sleep 3s，需数小时，可分多次 --limit 运行）
python scripts\backfill_album_profiles.py -d "D:\v2ph_archive"

# 只更新文字字段，跳过封面下载
python scripts\backfill_album_profiles.py -d "D:\v2ph_archive" --no-cover

# 【推荐两段走】第一段：先只补文字字段（跳过封面）
python scripts\backfill_album_profiles.py -d "D:\v2ph_archive" --no-cover
# 第二段：对 DB 中已有 cover_url 但未下载的行直接下载封面（无需再过 Cloudflare 页面拦截）
python scripts\backfill_album_profiles.py -d "D:\v2ph_archive" --cover-only

# 封面保存到自定义目录
python scripts\backfill_album_profiles.py -d "D:\v2ph_archive" --cover-dir "E:\covers"
